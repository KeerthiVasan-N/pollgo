...
func (device *Device) SaveIdentity(ctx context.Context, address *protocol.SignalAddress, identityKey *identity.Key) error {
	...
	key := identityKey.PublicKey().PublicKey()
	if putCachedIdentity(ctx, addrString, key) {
		return nil
	}
	err := device.Identities.PutIdentity(ctx, addrString, key)
	if err != nil {
...
func (device *Device) IsTrustedIdentity(ctx context.Context, address *protocol.SignalAddress, identityKey *identity.Key) (bool, error) {
	addrString := address.String()
	key := identityKey.PublicKey().PublicKey()
	if cachedKey, found, ok := getCachedIdentity(ctx, addrString); ok {
		if !found {
			// Not known → trust, will be persisted via SaveIdentity
			return true, nil
		}
		return cachedKey == key, nil
	}
	isTrusted, err := device.Identities.IsTrustedIdentity(ctx, addrString, key)
	if err != nil {
